...
import uz.android.creditCardNfcReader.utils.CommandApdu;
import uz.android.creditCardNfcReader.utils.ResponseUtils;
import uz.android.creditCardNfcReader.utils.TlvUtil;
import uz.android.creditCardNfcReader.utils.TrackUtils;
...
public class EmvParser {
    ...
    protected void readWithAID() throws CommunicationException {
        ...
        for (EmvCardScheme type : EmvCardScheme.values()) {
            for (byte[] aid : type.getAidByte()) {
                if (extractPublicData(aid, type.getmName())) {
                    return;
                }
            }
        }
    ...
    protected EmvCardScheme findCardScheme(final String pAid, final String pCardNumber) {
        ...
        if (type == EmvCardScheme.CB) {
            ...
            if (type != null) {
                LOGGER.debug("Real type:" + type.getmName());
            }
        }
        return type;
    }
